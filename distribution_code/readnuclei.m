function [M cellNames] = readnuclei(filename, col)
...
C = textscan(fid, '%f%d%d%f%f%f%f%f%f%s%f%*d%*d%*d%*s%*s', 'delimiter', ',');
fclose(fid);
...
M = [T, C{1},temp, C{6},C{7},C{8},C{9},C{11},C{4},C{5},double(C{3})];
cellNames = C{10};
if  any( C{2} == -1)
