...
%--------------------------------------------------------------------
%to test speed improvement using separability
%Fernando Amat September 17th 2010
siz2A=[floor(sigma/2)*2+1,floor(sigma/2)*2+1,max(3,floor(sigma/znonuniformity/2)*2+1)]-1;
sigA = siz2A/(4*sqrt(2*log(2)));
siz2B=[floor(sigma*1.6/2)*2+1,floor(sigma*1.6/2)*2+1,max(3,floor(sigma*1.6/znonuniformity/2)*2+1)]-1;
sigB = siz2B/(4*sqrt(2*log(2)));
filtered=imgaussianAnisotropy(stack,sigA,siz2A)-imgaussianAnisotropy(stack,sigB,siz2B);
