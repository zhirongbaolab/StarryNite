...
public class RungeKutta{
    	...
    	public static double cashKarp(DerivFunction g, double x0, double y0, double xn, double h, double abstol, double reltol, int maxiter){
        	...
        	while(x<xn){
            		i++;
	            	if(i>maxiter)throw new ArithmeticException("Maximum number of iterations exceeded");
        	    	dydx = g.deriv(x, y);
            		k1 = h*dydx;
            		...
            		err = Math.abs(y6 - y5);
            		tol= err/(Math.abs(y5)*reltol +  abstol);
            		if(tol<=1.0){
                		x  += h;
    	...
    	public static double[] cashKarp(DerivnFunction g, double x0, double[] y0, double xn, double h, double abstol, double reltol, int maxiter){
        	...
        	while(x<xn){
	            	...
	            	for(int i=0; i<nequ; i++)yd[i] = y[i] -11.0*k1[i]/54.0 + 5.0*k2[i]/2.0 - 70.0*k3[i]/27.0 + 35.0*k4[i]/27.0;
            		dydx = g.derivn(x + h, yd);
    	...
    	public static double[] fehlberg(DerivnFunction g, double x0, double[] y0, double xn, double h, double abstol, double reltol, int maxiter){
        	...
        	while(x<xn){
            		...
            		for(int i=0; i<nequ; i++)yd[i] = y[i] + k1[i]/4.0;
            		dydx = g.derivn(x + h/4.0, yd);
            		for(int i=0; i<nequ; i++)k2[i] = h*dydx[i];

            		for(int i=0; i<nequ; i++)yd[i] = y[i] + (3.0*k1[i] + 9.0*k2[i])/32.0;
            		dydx = g.derivn(x + 3.0*h/8.0, yd);
            		for(int i=0; i<nequ; i++)k3[i] = h*dydx[i];

            		for(int i=0; i<nequ; i++)yd[i] = y[i] + (1932.0*k1[i] - 7200.0*k2[i] + 7296.0*k3[i])/2197.0;
            		dydx = g.derivn(x + 12.0*h/13.0, yd);
            		for(int i=0; i<nequ; i++)k4[i] = h*dydx[i];

	            	for(int i=0; i<nequ; i++)yd[i] = y[i] + 439.0*k1[i]/216.0 - 8.0*k2[i] + 3680.0*k3[i]/513.0 - 845*k4[i]/4104.0;
            		dydx = g.derivn(x + h, yd);
            		for(int i=0; i<nequ; i++)k5[i] = h*dydx[i];

            		for(int i=0; i<nequ; i++)yd[i] = y[i] - 8.0*k1[i]/27.0 + 2.0*k2[i] - 3544.0*k3[i]/2565.0 + 1859.0*k4[i]/4104.0 - 11.0*k5[i]/40.0;
