...
public class TriCubicSpline{
        ...
        private String subMatrixIndices = " ";                  // String of indices of the submatrices that have called TriCubicSpline if called from higher dimension interpolation
    	...
    	// Set sub-matrix indices - for use with higher dimesion interpolations calling TriCubicSpline
    	...
    	public double interpolate(double xx1, double xx2, double xx3){
	    	...
	    	for (int i=0;i<nPoints;i++){
	    	    if(this.derivCalculated)this.bcsn[i].setDeriv(this.d2ydx2inner[i]);
		    	yTempm[i]=this.bcsn[i].interpolate(xx2, xx3);
		    	if(!this.derivCalculated)this.d2ydx2inner[i] = this.bcsn[i].getDeriv();
	    	}
