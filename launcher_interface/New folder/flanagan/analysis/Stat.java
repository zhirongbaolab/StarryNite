...
*            4-5 October 2008,  8-13 December 2008, 14 June 2009
*
*   DOCUMENTATION:
    ...
    public static double generalisedEntropyOneNat(double[] p, double q, double r){
       ...
       // Returns an array of Erlang random deviates - user supplied seed
        ...
        public static double normalCDF(double mean, double sd, double lowerlimit, double upperlimit){
            if(upperlimit==Double.POSITIVE_INFINITY){
                if(lowerlimit==Double.NEGATIVE_INFINITY){
                    ...
                    if(lowerlimit==Double.POSITIVE_INFINITY){
                        return 0.0;
                    }
                    else{
                        return (1.0 - Stat.normalCDF(mean, sd, lowerlimit));
                    }
                }
            }
            else{
        ...
        public static double gaussianInverseCDF(double mean, double sd, double prob){
            ...
            if(prob==0.0){
                ran = Double.NEGATIVE_INFINITY;
            }
            else{
                if(prob==1.0){
                    ran = Double.POSITIVE_INFINITY;
                }
                else{
                    ...
                    // call root searching method
                    ran = realR.bisect(gauss, lowerBound, upperBound);
                }
            }
       ...
       // Returns an array of maximal Gumbel (Type I EVD) random deviates - user supplied seed
        ...
        public static double weibullInverseCDF(double mu, double sigma, double gamma, double prob){
            ...
            if(prob==0.0){
                ...
                if(prob==1.0){
                    ...
                    ran = mu + sigma*(Math.pow(-Math.log(1.0 - prob), 1.0/gamma));
                }
            }
        ...
        // Weibull Inverse Cumulative Distribution Function; mu = 0
        ...
        // Weibull Inverse Cumulative Distribution Function; mu = 0, sigma = 1
        public static double weibullInverseCDF(double gamma, double prob){
            return weibullInverseCDF(0.0, 1.0, gamma, prob);
        }
        ...
        public static double exponentialInverseCDF(double mu, double sigma, double prob){
            ...
            if(prob==0.0){
                ...
                if(prob==1.0){
                    ...
                    ran = mu - sigma*(Math.log(1.0 - prob));
                }
            }
        ...
        public static double rayleighInverseCDF(double beta, double prob){
            ...
            if(prob==0.0){
                ...
                if(prob==1.0){
                    ...
                    ran = beta*(Math.sqrt(-Math.log(1.0 - prob)));
                }
            }
        ...
        public static double paretoInverseCDF(double alpha, double beta, double prob){
            ...
            if(prob==0.0){
                ...
                if(prob==1.0){
                    ...
                    ran = beta/Math.pow((1.0 - prob), 1.0/alpha);
                }
            }
        ...
        // STATISTICS FOR PROBABILITY PLOTS
        private static double[] probPlotStats(int flag, double[] param, double[] data){

            int nParam = param.length;
            int nData = data.length;

            double[] ret = new double[nParam+1];
            Stat st = new Stat(data);
            double range = st.maximum() - st.minimum();
            double delta = 1e-3;    // step fraction in numerical differentiarion

            switch(flag){
                case 0: // Gaussian Probability Plot
                        GaussProbPlotFunc func = new GaussProbPlotFunc();

                        func.setDataArray(data);
                        double sumOfSquares = func.function(param);

                        double[][] secondDeriv = new double[2][2];
                        double[] paramTemp = new double[2];
                        double pari = 0.0;
                        double parj = 0.0;
                        for(int i=0; i<2; i++){
                            for(int j=0; j<2; j++){
                                if(i==j){
                                    paramTemp = param.clone();
                                    paramTemp[i] = param[i]*(1.0 + delta);
                                    pari = param[i];
                                    if(i==0){
                                        if(param[i]==0.0)paramTemp[i] = +param[1]*delta/10.0;
                                        pari = param[1]/10.0;
                                        if(param[i]==0.0)paramTemp[i] = +range*delta/100.0;
                                        pari = range/100.0;
                                    }
                                    else{
                                        if(param[i]==0.0)paramTemp[i] = +3.0*param[0]*delta;
                                        pari = 3.0*param[0];
                                        if(param[i]==0.0)paramTemp[i] = +range*delta/35.0;
                                        pari = range/35.0;
                                    }

                                    double term1 = func.function(paramTemp);
                                    System.out.println("1 " + paramTemp[0] + " " + paramTemp[1]);
                                    ...
                                    double term2 = func.function(paramTemp);
                                    System.out.println("2 " + paramTemp[0] + " " + paramTemp[1]);

                                    paramTemp = param.clone();
                                    paramTemp[i] = param[i];
                                    pari = param[i];

                                    double term3 = func.function(paramTemp);
                                    System.out.println("3 " + paramTemp[0] + " " + paramTemp[1]);
                                    ...
                                    double term4 = func.function(paramTemp);
                                    System.out.println("4 " + paramTemp[0] + " " + paramTemp[1]);

                                    System.out.println(term1 + " " + term2 + " " + term3 + " " + term4);
                                    secondDeriv[i][j] = (term1 - term2 - term3 + term4)/(pari*pari*delta*delta);
                                    ...
                                    paramTemp = param.clone();
                                    paramTemp[i] = param[i]*(1.0 + delta/2.0);
                                    pari = param[i];
                                    if(i==0){
                                        if(param[i]==0.0)paramTemp[i] = +param[1]*delta/20.0;
                                        pari = param[1]/10.0;
                                        if(param[i]==0.0)paramTemp[i] = +range*delta/200.0;
                                        pari = range/100.0;
                                    }
                                    else{
                                        if(param[i]==0.0)paramTemp[i] = +3.0*param[0]*delta/2.0;
                                        pari = 3.0*param[0];
                                        if(param[i]==0.0)paramTemp[i] = +range*delta/70.0;
                                        pari = range/35.0;
                                    }
                                    paramTemp[j] = param[j]*(1.0 + delta/2.0);
                                    parj = param[j];
                                    if(j==0){
                                        if(param[j]==0.0)paramTemp[j] = +param[1]*delta/20.0;
                                        parj = param[1]/10.0;
                                        if(param[j]==0.0)paramTemp[j] = +range*delta/200.0;
                                        parj = range/100.0;
                                    }
                                    else{
                                        if(param[j]==0.0)paramTemp[j] = +3.0*param[0]*delta/2.0;
                                        parj = 3.0*param[0];
                                        if(param[j]==0.0)paramTemp[j] = +range*delta/70.0;
                                        parj = range/70.0;
                                    }
                                    double term1 = func.function(paramTemp);
                                    System.out.println("1 " + paramTemp[0] + " " + paramTemp[1]);
                                    ...
                                    System.out.println(term1 + " " + term2 + " " + term3 + " " + term4  +  " " + (term1 - term2 - term3 + term4));
                                    secondDeriv[i][j] = (term1 - term2 - term3 + term4)/(pari*parj*delta*delta);
                                }
                            }
                        }

                        PrintToScreen.print(secondDeriv);
                        Matrix mat = new Matrix(secondDeriv);
                        mat = mat.inverse();
                        mat = mat.times(sumOfSquares/(nData - nParam));
                        for(int i=0; i<2; i++)ret[i] = Math.sqrt(mat.getElement(i,i));
                        ret[2] = sumOfSquares;
                        break;
            }
