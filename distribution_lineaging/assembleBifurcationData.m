...
if(isempty(d1endcand))
    ...
    if(origparent~=-1)%option originating in a division
        ...
        %candidate pair
        [~,~,flengthd1, solidlengthd1]=computeConfidenceRecursive(esequence,d1endcandt(minid1),d1endcand(minid1),trackingparameters);
...
if(isempty(d2endcand))
    ...
    if(origparent~=-1) %option orig in division 
        ...
        [~,~,flengthd2, solidlengthd2]=computeConfidenceRecursive(esequence,d2endcandt(minid2),d2endcand(minid2),trackingparameters);
...
%note it doesn't make sense to use the distance scoring here so this calls
%the code directly rather than using parameter function handle
[~,splitscores_div,~,features]=...
    divScoreModelCostFunction(esequence,i,t,esequence{t}.suc(i,:),esequence{t}.suc_time(i,:), trackingparameters);
if(trackingparameters.recordanswers)
...
%new avoiding global vector reference
if(minsize==d1length)
...
alldaughterdata=[features,confidenceData.bifconvector(count-1,:),minsize,ncells];
